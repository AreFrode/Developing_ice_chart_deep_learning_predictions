...
\begin{tikzpicture}[x={(1,0)},y={(0,1)},z={({cos(60)},{sin(60)})},
  ...
  \node (sic_pred) at (unet -| aa_mask) {\includegraphics[width=.25\textwidth]{20220105.pdf}};
  \node [above = 0cm of sic_pred] (sic_pred_name) {\large Predicted Ice Chart};
  \draw [line width=0.8mm, -{Stealth[length=8mm, round]}, shorten >= 0.1cm, shorten <= 0.1cm] (unet.east) -- (sic_pred.west);
